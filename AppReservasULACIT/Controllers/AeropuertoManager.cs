...
namespace AppReservasULACIT.Controllers
{
    public class AeropuertoManager
    {
        ...
        public async Task<Aeropuerto> ObtenerAeropuerto(string token, string codigo)
        {
            HttpClient httpClient = GetClient(token);
        ...
        public async Task<Aeropuerto> Ingresar(Aeropuerto aeropuerto, string token)
        {
            ...
            var response = await httpClient.PostAsync(Url,
                new StringContent(JsonConvert.SerializeObject(aeropuerto), Encoding.UTF8, "application/json"));
